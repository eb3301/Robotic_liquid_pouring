...
def show(Im,txt=""):
    ...
    # Use a more compatible approach to maximize the window
    try:
        # Try the old method first
        figManager.window.showMaximized()
    except AttributeError:
        try:
            # Try alternative method for newer tkinter versions
            figManager.window.state('zoomed')
        except (AttributeError, Exception):
            try:
                # Try another alternative
                figManager.window.attributes('-zoomed', True)
            except (AttributeError, Exception):
                try:
                    # Try another alternative for some systems
                    figManager.window.attributes('-fullscreen', True)
                except (AttributeError, Exception):
                    # If all else fails, just resize to a large size
                    try:
                        figManager.resize(1920, 1080)
                    except:
                        pass  # If even this fails, just continue without resizing
